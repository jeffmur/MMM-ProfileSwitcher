...
Module.register("MMM-ProfileSwitcher", {
  ...
  isVisible: function (self, useEveryone, classes) {
    return (
      classes.indexOf(self.current_profile) !== -1 || // Does this module include the profile?
      (useEveryone && classes.indexOf(self.config.everyoneClass) !== -1)
    ); // Should everyone see this module?
  },
    ...
    modules.enumerate(function (module) {
      if (self.isVisible(self, useEveryone, module.data.classes.split(" "))) {
        module.show(
          ...
          function () {
            console.log(module.name + " is shown.");
            if (module.name === "MMM-GoogleAssistant")
              self.sendNotification("ASSISTANT_START", null);
          },
          options
        );
      } else {
        module.hide(
          ...
          function () {
            ...
            if (module.name === "MMM-GoogleAssistant")
              self.sendNotification("ASSISTANT_STOP", null);
          },
          options
        );
      }
  ...
  change_profile: function (newProfile) {
    ...
    if (newProfile !== this.current_profile) {
      this.sendNotification("CHANGED_PROFILE", {
        from: this.current_profile,
        to: newProfile
      });

      if (newProfile === this.config.defaultClass) {
        console.log("Changing to default profile.");
        ...
        console.log("Changing to profile " + newProfile + ".");

        if (
          this.config.alwaysShowLeave &&
  ...
  socketNotificationReceived: function (notification, payload) {
    ...
    if (notification === "USER_DETECTED") {
      ...
      if (status === "ACTIVE") {
        // Notify MMM-DeviceControl to wakeup the screen
        this.sendNotification("WAKEUP", undefined);
        this.change_profile(profile);
      } else if (status === "LEFT") {
        this.change_profile(this.config.defaultClass);
      }
    }
  },
  ...
  notificationReceived: function (notification, payload, sender) {
    ...
    if (notification === "DOM_OBJECTS_CREATED") {
      console.log("Hiding all non default modules.");
      this.set_profile(this.config.includeEveryoneToDefault);
      this.sendNotification("CHANGED_PROFILE", {
        to: this.config.defaultClass
      });
  ...
  start: function () {
    ...
    this.config.enterMessages = this.parseMessages(
      this.config.enterMessages,
      this.translate("enter")
    );

    // Parse the Message data from leave and add everyone if needed
    this.config.leaveMessages = this.parseMessages(
      this.config.leaveMessages,
      this.translate("leave")
    );

    console.info("Starting module: " + this.name);
    this.sendSocketNotification("START", undefined);
  },
